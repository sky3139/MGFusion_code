...
struct TSDF {
    ...
    void loop()
    {
        ...
        cv::Mat points, color;
        // parser = new FrParser(0, -1, 1);
        // assert(parser->OpenFile("/home/u16/dataset/cafe1-1/"));
        ...
        //
        DeviceArray2D<unsigned short> depth_device(480, 640);
        ...
        parser=new dataset_tum(0,-1,1,fs["matpose"]);
        // act act_(0,-1,1,fs["matpose"]);
        if(mode==2)
        {
            mm.loadfile(mm.points,mm.color);
            mp_v->inset_cloud("gloab", cv::viz::WCloud(mm.points,mm.color));//)); // ));
            while (1) {
        ...
        if(mode==3)
        {
            ...
            dataset::Mat_read_binary(pt, "points.cvmat");
            dataset::Mat_read_binary(color, "color.cvmat");
            mm.cloud2tsdf(pt,color,expoints,excolor);
        ...
        // bool tf = false;
        for (int frame_idx = first_frame_idx; frame_idx < first_frame_idx + (int)num_frames; frame_idx += 1) { // ++frame_idx
            // tm.Start();
            bool over_= parser->ReadNextTUM(frame_idx);
            // parser->rgb_=act_.rgb_;
            // parser->depth_src=act_.depth_src.clone();
            // act_.ReadNextTUM(frame_idx);
            if(!over_)
            {
                break;
            }
            ...
            depth_device_by.upload(parser->depth_src.data, parser->depth_src.step, parser->depth_src.rows, parser->depth_src.cols);

            bilateralFilter2( depth_device_by,depth_device,7,4.5f,0.04f);
            cv::Mat point_tsdf;
            ...
            device::scaleDepth <<< grid_scale, block_scale>>>(depth_device, depthScaled,gocloud,p_int,g_cam, intr);
            checkCUDA(cudaGetLastError());


            computePointNormals(intr, depth_device,points_pyr,normals_pyr );

            renderImage(intr, depth_device,points_pyr,normals_pyr,imgcuda);
            // float4 *pbuf,*pnormal;
            // cudaMallocHost((void **)&pnormal, sizeof(float4)*640*480);

            // cudaMallocHost((void **)&pbuf, sizeof(float4)*640*480);
            // points_pyr.download(pbuf,sizeof(float4)*640);
            // normals_pyr.download(pnormal,sizeof(float4)*640);
            // for(int i=0; i<640*480; i++)
            // {
            //     if(isnan(pbuf[i].x))
            //         continue;
            //     // if(std::is_nan(pbuf[i].y))
            //     //     continue;
            //     // if(std::is_nan(pbuf[i].z))
            //     //     continue;
            //     // if(std::is_nan(pnormal[i].x))
            //     //     continue;
            //     // if(std::is_nan(pnormal[i].y))
            //     //     continue;
            //     // if(std::is_nan(pnormal[i].z))
            //     //     continue;
            //     printf("%f %f %f %f %f %f\n",pbuf[i].x,pbuf[i].y,pbuf[i].z,pnormal[i].x,pnormal[i].y,pnormal[i].z);
            // }
            // assert(0);
// DeviceArray2D<float4> points_pyr,normals_pyr;
            ...
            // parser->m_pose.val[3] -= 10;
            // parser->m_pose.val[7] -= -1.4;
            // parser->m_pose.val[11] -= 1.4;
            ...
            // depthScaled.download(host_points,1);

            // tm.Start();
            // cv::Mat dep = getCload(parser->depth_, point_tsdf, cubexs, cam_pose, host_points, mm.cpu_kpara);
            // tm.PrintSeconds("getCload");
            // tm.Start();
            ...
            //移除
            if (frame_idx % 100 == 15 || mm.gpu_pbox_free.size() < 100)
            {
                tm.Start();
                mm.savenode_cube_();
